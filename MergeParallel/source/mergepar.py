...
def merge_sort_parallel(data, core=0):
    ...
    processes = multiprocessing.cpu_count() if core == 0 else core
    pool = multiprocessing.Pool(processes=processes)
        
    size = int(math.ceil(len(data) / processes))
...
if __name__ == '__main__':
    ...
    for i in range(1, 9):
        start = time.time()
        data_sorted = merge_sort_parallel(data_unsorted, i)
        end = time.time() - start
